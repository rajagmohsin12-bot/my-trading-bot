...
class MarketDataFetcher:
    def __init__(self):
        self.symbols = {'BTC-USD': 'Bitcoin', 'ETH-USD': 'Ethereum', 'EURUSD=X': 'EUR/USD', 'GBPUSD=X': 'GBP/USD', 'GC=F': 'Gold'}
...
st.set_page_config(page_title="ALPHA QUANT - Institutional Terminal", layout="wide")
data_fetcher = MarketDataFetcher()
st.title("⚡ ALPHA QUANT TERMINAL")
st.markdown("### Institutional Multivariable Market Matrix")
asset_selection = st.sidebar.selectbox("Select Target Core Asset:", list(data_fetcher.symbols.keys()))
time_frame = st.sidebar.selectbox("Analytical Timeframe:", ["1h", "1d"])

try:
    with st.spinner("Decoding Deep Market Flow Matrix..."):
        df = data_fetcher.fetch_data(asset_selection, interval=time_frame)
    if df is None or df.empty or len(df) < 30:
        st.error("API Latency Timeout or Dynamic Data Frame Corruption.")
    else:
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)
        kalman_engine = KalmanFilterArbitrage()
        vol_engine = VolatilityBreakoutStrategy()
        flow_engine = OrderFlowImbalanceStrategy()
        scorer = InstitutionalCertaintyScorer()
        k_signal = kalman_engine.filter_signal(df['Close'].values)
